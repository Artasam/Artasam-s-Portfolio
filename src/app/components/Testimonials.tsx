      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-6">
          <motion.div
            ...
            className="max-w-2xl"
          >
            <h2 className="text-sm font-bold text-rose-600 uppercase tracking-widest mb-4">Engineering Excellence</h2>
          ...
          {achievements.map((achievement, index) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                className="group relative bg-[#0d0d12] border border-zinc-800 rounded-3xl p-8 hover:border-rose-600/30 transition-all"
              >
                <div className="w-12 h-12 bg-zinc-900 rounded-xl flex items-center justify-center text-zinc-500 group-hover:text-rose-500 group-hover:bg-rose-500/10 transition-all mb-6">
                  <Icon className="w-6 h-6" />
                </div>
                
                <span className="text-[10px] font-bold text-rose-600 uppercase tracking-widest mb-2 block">
                  {achievement.category}
                </span>
                <h4 className="text-xl font-bold text-white mb-2 group-hover:text-rose-500 transition-colors">
                  {achievement.title}
                </h4>
                <div className="text-2xl font-black text-zinc-600 mb-4 group-hover:text-white transition-colors">
                  {achievement.metric}
                </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <p className="text-zinc-500 text-lg mb-10 font-medium max-w-lg">
              Specialized expertise in building the next generation of reasoning-driven AI systems and scalable ML infrastructures.
            </p>
            
            <div className="inline-flex items-center gap-3 px-6 py-4 bg-zinc-900 border border-zinc-800 rounded-2xl">
              <ShieldCheck className="text-rose-600 w-5 h-5" />
              <span className="text-sm font-bold text-zinc-300 uppercase tracking-widest">Enterprise Ready</span>
            </div>
          </motion.div>

          <div className="space-y-8 bg-[#0d0d12] border border-zinc-800 rounded-3xl p-8 md:p-12">
            {skills.map((skill, index) => (
              <div key={skill.name} className="space-y-3">
                <div className="flex justify-between items-end">
                  <span className="text-sm font-bold text-white uppercase tracking-widest">{skill.name}</span>
