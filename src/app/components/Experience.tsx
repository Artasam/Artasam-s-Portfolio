      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-20">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-sm font-bold text-rose-600 uppercase tracking-widest mb-4">Professional History</h2>
        ...
        <div className="space-y-12">
          {experiences.map((exp, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group relative bg-[#0d0d12] border border-zinc-800 rounded-3xl p-8 md:p-12 hover:border-rose-600/30 transition-all"
            >
              <div className="flex flex-col lg:flex-row gap-12">
                ...
                <div className="lg:w-1/3">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      <h4 className="text-2xl font-bold text-white group-hover:text-rose-500 transition-colors">{exp.company}</h4>
                      <p className="text-rose-600 font-bold text-xs uppercase tracking-widest">{exp.role}</p>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-4">
                    {exp.achievements.map((achievement, i) => (
                      <li key={i} className="flex items-start gap-4 text-zinc-400 group/item">
                        <CheckCircle2 className="w-5 h-5 text-zinc-700 group-hover/item:text-rose-600 transition-colors flex-shrink-0 mt-1" />
                        <span className="text-lg leading-relaxed font-medium group-hover/item:text-zinc-200 transition-colors">{achievement}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {/* Decorative Number */}
              <div className="absolute top-8 right-12 text-7xl font-black text-zinc-900 group-hover:text-rose-950 transition-colors select-none pointer-events-none">
                0{index + 1}
              </div>
            </motion.div>
          ))}
